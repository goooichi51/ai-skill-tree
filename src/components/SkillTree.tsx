...
function calculateNodePositions(nodes: Node[], genres: Genre[], centerX: number, centerY: number) {
  ...
  const genreColors = genres.map(g => g.color);
  const angleStep = (2 * Math.PI) / Math.max(nodes.length, 1);
  const baseRadius = 200;

  nodes.forEach((node, index) => {
    const angle = angleStep * index - Math.PI / 2;
    const levelRadius = baseRadius + (node.level - 1) * 80;
    positions[node.id] = {
      x: centerX + Math.cos(angle) * levelRadius,
      y: centerY + Math.sin(angle) * levelRadius,
      color: genreColors[index % genreColors.length] || '#6366f1'
    };
  });
  return positions;
}
...
export default function SkillTree({ genres, nodes, useCases }: SkillTreeProps) {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 p-6">
      <div className="flex-1 bg-slate-800/50 rounded-2xl p-4 overflow-hidden">
        <svg width="100%" height="600" viewBox={`0 0 ${svgWidth} ${svgHeight}`} className="max-w-full">
          <defs>
          ...
          <g transform={`translate(${centerX}, ${centerY})`}>
            <circle r="50" fill="#1e293b" stroke="#6366f1" strokeWidth="4" />
            <circle r="40" fill="url(#coreGradient)" />
          ...
          {nodes.map((node) => {
            ...
            return (
              <line key={`line-${node.id}`} x1={centerX} y1={centerY} x2={pos.x} y2={pos.y}
                stroke="#6366f1" strokeWidth="2" strokeOpacity="0.3" strokeDasharray="5,5" />
            );
          })}
          ...
          {nodes.map((node) => {
            ...
            return (
              <SkillNode key={node.id} node={node} x={pos.x} y={pos.y} color={pos.color}
                isUnlocked={true} onClick={setSelectedNode} />
            );
          })}
        </svg>
      </div>
      ...
      <div className="w-full lg:w-80 space-y-4">
        ...
        {selectedNode && (
          <div className="bg-slate-800/50 rounded-2xl p-4">
            ...
            <div className="text-xs text-indigo-400 mb-2">レベル {selectedNode.level}</div>
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto">
            {useCases.map((useCase) => (
              <div key={useCase.id}
                ...
                {selectedUseCase?.id === useCase.id && <p className="text-xs text-gray-300 mt-1">{useCase.description}</p>}
              </div>
            ))}
          </div>
        </div>
